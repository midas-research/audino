...
export default function ConflictsList({
    ...
    const getAllAnnotationsFromGroundTruthId = useGetAllAnnotation({
        queryConfig: {
            ...
            staleTime: Infinity,
            onSuccess: (data) => {
                generateMissingRegionsFromGroundTruth(conflicts, data?.shapes);
                                ...
                                {conflicts.map((conflict, index) => {
                                    return (
                                        <div
                                            ...
                                            className={`flex  border-l-4 flex-col ${conflict?.type !== "missing_annotation"
                                                ? " cursor-pointer"
                                                : "cursor-not-allowed"
                                                } py-2 my-2 justify-center shadow rounded p-2 ${selectedSegment?.id === getAnnotationId(conflict)
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                <span
                                                    className={`inline-flex items-center rounded-md ${conflict?.severity === "error"
