...
export default function AttributeFields({
  ...
  const [showEmojiList, setShowEmojiList] = useState(
    Array.from({ length: value.values.length }, () => false)
  );
  const { debouncedValidation } = useSingleFieldValidation(
    attributeSingleFieldValidation,
    1000,
    attributesError[index],
    (val) => {
  ...
  const handleRemoveValue = (valueIndex) => {
    ...
    setShowEmojiList((prev) =>
      prev.filter((val, index) => index !== valueIndex)
    );
  };
...
//   console.log("AttributeFields rendered", index, value);
  return (
    <div className="w-full" key={`newAttribute-${index}`}>
      ...
      <fieldset className="space-y-2 px-4 sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:space-y-0 sm:px-6 sm:py-5">
        ...
        <div
          className="text-sm font-medium leading-6 dark:text-audino-light-gray text-gray-900"
          aria-hidden="true"
        >
          Attribute values
        </div>
        <div className="space-y-1 sm:col-span-2">
          {value?.values?.length ? (
            <div className="relative flex items-start flex-wrap gap-2 mb-2">
              {/* value */}
          ...
          {attributesError[index] &&
            attributesError[index]["attribute_values"] && (
      ...
      {value?.id ? null : (
        <div
          className="flex gap-1 items-center justify-end mr-6 mb-6 text-red-400 hover:text-red-500 hover:cursor-pointer"
