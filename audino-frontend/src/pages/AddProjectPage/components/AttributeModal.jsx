...
import { useUpdateLabelMutation } from "../../../services/Labels/useMutation";
...
export default function AttributeModal({
  ...
  const handleCreate = () => {
    ...
    if (isReadyToMerge) {
      if (currentLabelIndex >= 0) {
        ...
        if (labels_obj.results[currentLabelIndex]?.id) {
          setIsLoading(true);
          updateLabelMutation.mutate({
            id: updatedLabels[currentLabelIndex].id,
            data: {
              ...updatedLabels[currentLabelIndex],
            },
          });
        } else {
          handleCancel();
  ...
  const handleRemoveAttributeValue = (attribIndex, valueIndex) => {
    ...
    updatedAttribValues = updatedAttribValues.filter(
  ...
  useEffect(() => {
    if (
      currentLabelIndex >= 0 &&
      labels_obj.results[currentLabelIndex].attributes.length
    ) {
                ...
                <Dialog.Panel className="pointer-events-auto w-screen max-w-2xl">
                  <form className="flex h-full flex-col overflow-y-scroll dark:bg-audino-navy bg-white shadow-xl">
                    <div className="flex-1">
                        ...
                        <div className="flex items-start justify-between space-x-3">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-200">
                              Click on add new attribute button below to create
                              your new attribute.
                            </p>
                          </div>
                      ...
                      {attributes.length ? (
                        <div className="relative flex items-start flex-wrap gap-2 mb-2 space-y-6 py-6 sm:space-y-0 sm:divide-y sm:divide-gray-200 dark:divide-audino-graphite sm:py-0">
                          {attributes.map((attr, index) => (
                            <AttributeFields
                              key={`attributefields-${index}`}
                              index={index}
                              onRemoveValue={(i) =>
                                handleRemoveAttributeValue(index, i)
                              }
                              onRemoveAttribute={() =>
                                handleRemoveAttribute(index)
                              }
                              onInputChange={(name, val) =>
                                handleAttributeFieldChange(name, val, index)
                              }
