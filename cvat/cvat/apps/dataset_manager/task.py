...
from cvat.apps.engine.models import Job, AttributeSpec, JobType, TaskFlags
...
def create_annotation_clips_zip(annotation_audio_chunk_file_paths, meta_data_file_path, output_folder, dst_file, save_clips = True):
    data_folder = os.path.join(output_folder, 'data')
    clips_folder = os.path.join(data_folder, 'clips')
    os.makedirs(data_folder, exist_ok=True)

    # Optionally copy audio files to clips folder
    if save_clips:
...
def create_gt_annotation_clips_zip(annotation_audio_chunk_file_paths, json_data, temp_dir_base, dst_file, save_clips = True):
    data_folder = os.path.join(temp_dir_base, 'data')
    clips_folder = os.path.join(data_folder, 'clips')
    os.makedirs(data_folder, exist_ok=True)
    ...
    zip_filename = os.path.join(temp_dir_base, 'common_voice.zip')
    with zipfile.ZipFile(zip_filename, 'w') as zipf:
        for root, _, files in os.walk(data_folder):
            for file in files:
                file_path = os.path.join(root, file)
...
def get_audio_job_export_data(job_id, dst_file, job, temp_dir_base, temp_dir, audio_files_mapping):
    ...
    def process_annotations(annotations, gt_jobs, job_details, labels_mapping):
        ...
        for i, annotation in enumerate(annotations):
            ...
            if isinstance(job_details.segment.audio_file_index, int) and job_details.segment.audio_file_index >= 0:
                entry['audio_file'] = audio_files_mapping[job_details.segment.audio_file_index]
            else:
                entry['audio_file'] = ""
...
def convert_annotation_data_format(job, data, format_name):
    if format_name == "Common Voice":
        return data
    elif format_name == "Audino Format":
        task_flags = TaskFlags.objects.get(task_id=job.db_job.segment.task.id)
        formatted_data = []
        ...
        for entry in data:
            ...
            if task_flags.is_start:
                formatted_entry["start"] = entry.get("start")

            if task_flags.is_end:
                formatted_entry["end"] = entry.get("end")

            if task_flags.is_transcription:
                formatted_entry["text"] = entry.get("sentence")

            if task_flags.is_gender:
                formatted_entry["gender"] = entry.get("gender")

            if task_flags.is_locale:
                formatted_entry["language"] = entry.get("locale")

            if task_flags.is_accent:
                formatted_entry["accent"] = entry.get("accent")

            if task_flags.is_age:
                formatted_entry["age"] = entry.get("age")

            if task_flags.is_emotion:
                formatted_entry["emotion"] = entry.get("emotion")
...
def get_audio_file_mapping(raw_path):
    all_audio_files = []
    items = os.listdir(raw_path)

    if len(items) == 1 and os.path.isdir(os.path.join(raw_path, items[0])):
        # Case: Single folder in 'raw' containing audio files
        folder_path = os.path.join(raw_path, items[0])
        for item in os.listdir(folder_path):
            file_path = os.path.join(folder_path, item)
            if os.path.isfile(file_path):
                all_audio_files.append(os.path.basename(file_path))
    else:
        # Case: Multiple files directly in 'raw'
        for item in items:
            file_path = os.path.join(raw_path, item)
            if os.path.isfile(file_path):
                all_audio_files.append(os.path.basename(file_path))

    return sorted(all_audio_files, key=str.lower)
...
def export_audino_job(job_id, dst_file, format_name, server_url=None, save_images=False):
    ...
    audio_files_mapping = get_audio_file_mapping(os.path.join(job.db_job.segment.task.data.get_data_dirname(), "raw"))

    os.makedirs(temp_dir_base, exist_ok=True)

    with TemporaryDirectory(dir=temp_dir_base) as temp_dir:

        final_data, annotation_audio_chunk_file_paths = get_audio_job_export_data(job_id, dst_file, job, temp_dir_base, temp_dir, audio_files_mapping)

        # Convert the data into a format
        final_data = convert_annotation_data_format(job, final_data, format_name)

        if job.db_job.type == JobType.GROUND_TRUTH:
            time_stamps = job.db_job.segment.time_stamps
            json_data = {
                "annotations" : final_data,
                "time_stamps" : time_stamps
            }
            create_gt_annotation_clips_zip(annotation_audio_chunk_file_paths, json_data, temp_dir_base, dst_file, save_clips=False if format_name == "Audino Format" else True)
        else:
            ...
            create_annotation_clips_zip(annotation_audio_chunk_file_paths, meta_data_file_path, temp_dir_base, dst_file, save_clips=False if format_name == "Audino Format" else True)
...
def export_audino_task(task_id, dst_file, format_name, server_url=None, save_images=False):
    ...
    audio_files_mapping = get_audio_file_mapping(os.path.join(task.db_task.data.get_data_dirname(), "raw"))
    ...
    with TemporaryDirectory(dir=temp_dir_base) as temp_dir:
        for job in task_jobs:
            ...
            final_data, annotation_audio_chunk_file_paths = get_audio_job_export_data(job.db_job.id, dst_file, job, temp_dir_base, temp_dir, audio_files_mapping)
        ...
        # sorting by job_id column in pandas dataframe
        # df = df.sort_values(by='job_id')

        df.to_csv(meta_data_file_path, sep='\t', index=False)

        create_annotation_clips_zip(final_annotation_chunk_paths_flatten, meta_data_file_path, temp_dir_base, dst_file, save_clips=False if format_name == "Audino Format" else True)
