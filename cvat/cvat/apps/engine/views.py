...
from django.http import HttpResponse, HttpRequest, HttpResponseNotFound, HttpResponseBadRequest, JsonResponse, StreamingHttpResponse, Http404
...
from cvat.apps.engine.background_operations import GroundTruthBackgroundJob
    ...
    def __call__(self, request, start: int, stop: int, db_data: Optional[Data]):
        ...
        try:
            if self.type == 'chunk':
                ...
                # Follow symbol links to get the real file path.
                ...
                if not os.path.exists(path):
                    raise NotFound(detail='Cannot find requested chunk file.')

                # Use our new streaming function instead of sendfile.
                return stream_response_generator(request, path)
...
class TaskViewSet(viewsets.GenericViewSet, mixins.ListModelMixin,
        ...
        def _handle_upload_data(request):
            with transaction.atomic():
                ...
                if data['client_files']:
                    data['client_files'] = sorted(data['client_files'], key=str.lower)
    ...
    @extend_schema(
        summary='Get the creation status of a GT job',
        description='Retrieves the status of a background job responsible for GT job creation of task with pk task id.',
        responses={
            '200': RqStatusSerializer
        },
    )
    @action(detail=True, methods=['GET'], serializer_class=RqStatusSerializer)
    def gt_status(self, request, pk):
        self.get_object() # force call of check_object_permissions()
        response = self._get_gt_rq_response(
            queue=settings.CVAT_QUEUES.GT_JOBS.value,
            job_id=RQIdManager.build('create', 'gt_job', pk)
        )
        serializer = RqStatusSerializer(data=response)
    ...
    @staticmethod
    def _get_gt_rq_response(queue, job_id):
        rq_job = django_rq.get_queue(queue).fetch_job(job_id)
        response = {}
        ...
        if rq_job is None:
            ...
            response = {
                "state": "Failed",
                "message": parse_exception_message(rq_job.exc_info or "Unknown error")
            }
        elif rq_job.is_queued or rq_job.is_deferred:
            response = { "state": "Queued" }
        elif rq_job.is_finished:
            app_status = rq_job.meta.get(RQJobMetaField.STATUS)
            if app_status == 'COMPLETED':
                response = { "state": "Finished" }
            elif app_status == 'FAILED':
                response = {
                    "state": "Failed",
                    "message": rq_job.meta.get('error_message', 'Task failed internally after RQ reported completion.')
                }
            else:
                response = {
                    "state": "Started",
                    "message": app_status or "Finalizing operation"
                }
        else:
            response = { "state": "Started" }
            response['message'] = rq_job.meta.get(RQJobMetaField.STATUS, 'Processing')
            response['progress'] = rq_job.meta.get(RQJobMetaField.PROGRESS, 0.0)
...
def read_file_chunks(file_path, start_byte, chunk_size, buffer_size=8192):
    """Helper generator to read a specific portion of a file in chunks."""
...
def stream_response_generator(request, file_path):
    """
    Creates a streaming HTTP response that supports Range requests for a given file.
    """
    ...
    if range_header:
        ...
        response = StreamingHttpResponse(
            streaming_content=read_file_chunks(file_path, start_byte, content_length),
        ...
        # Stream the whole file if no Range header is present
        response = StreamingHttpResponse(
            streaming_content=read_file_chunks(file_path, 0, file_size),
            status=200, # OK
            content_type=mime_type
        )
...
@extend_schema_view(
    create=extend_schema(
        ...
        responses={
            ...
            '202': OpenApiResponse(description="Ground Truth job creation initiated. Check status using rq_id."), # added 202 response.
        }),
    retrieve=extend_schema(
...
class JobViewSet(viewsets.GenericViewSet, mixins.ListModelMixin, mixins.CreateModelMixin,
    ...
    def perform_create(self, serializer):
        ...
        if validated_data["type"] == JobType.GROUND_TRUTH:
            task_id = validated_data.pop('task_id')
            queue = GroundTruthBackgroundJob.get_queue()
            rq_id = GroundTruthBackgroundJob.get_rq_id(task_id)
            queue.enqueue(GroundTruthBackgroundJob.create_gt_job_task, task_id, validated_data, job_id=rq_id)
            serializer.context['rq_id'] = rq_id
            serializer.context['job_status'] = 'queued'
            return Response(data={"message": "Ground Truth job creation initialized.", "rq_id": rq_id}, status=status.HTTP_202_ACCEPTED)
        else:
    ...
    def data(self, request, pk):
        ...
        job_ids = list(models.Job.objects.filter(
                            segment__task_id=db_job.segment.task_id,
                            type=models.JobType.ANNOTATION.value
                        ).values_list('id', flat=True).order_by('id'))

        if db_job.type == models.JobType.GROUND_TRUTH:
            data_num = len(job_ids)
        else:
            data_num = db_job.id - job_ids[0]
    ...
    @extend_schema(summary='Get audio peaks of a job',
    parameters=[],
    responses={
        '200': OpenApiResponse(OpenApiTypes.BINARY, description='Data of a specific type'),
    })
    ...
    def peaks_data(self, request, pk):
        db_job = self.get_object() # call check_object_permissions as well
        job_ids = list(models.Job.objects.filter(
                            segment__task_id=db_job.segment.task_id,
                            type=models.JobType.ANNOTATION.value
        ...
        path = os.path.realpath(frame_provider.get_chunk(data_num, FrameProvider.Quality.COMPRESSED))

        if not os.path.exists(path):
            raise Http404("Audio data not found")
        else:
            path = path.replace('compressed', 'peaks').replace('.mp3', '.json')
            if not os.path.exists(path):
                raise Http404("Audio peaks data not found")
    ...
    def metadata(self, request, pk):
        ...
        db_data.time_stamps = db_job.segment.time_stamps
...
class AIAudioAnnotationViewSet(viewsets.ModelViewSet):
    ...
    def save_segments(self, request):
        try:
            ...
            job.ai_audio_annotation_lang = request.data.get("ai_annotation_lang", "")
