...
class SegmentParams(NamedTuple):
    start_frame: int
    stop_frame: int
    audio_file_index: Optional[int] = None
...
def _get_task_segment_data(
    ...
    *,
    ...
    if segment_duration is not None:
        ...
        overlap = db_task.overlap or 0

        total_frames_list = db_task.total_frames_count
        total_duration_list = db_task.audio_total_duration

        def _segments():
            for idx, frame_count in enumerate(total_frames_list):
                duration_ms = total_duration_list[idx]
                if duration_ms == 0:
                    continue

                frames_per_ms = frame_count / duration_ms
                segment_duration = min(duration_ms, db_task.segment_duration)
                segment_size = min(int(frames_per_ms * segment_duration), frame_count)
                audio_file_index = idx

                if segment_size == 0:
                    raise ValueError("Segment size cannot be zero.")

                for start_frame in range(0, frame_count - overlap, segment_size - overlap):
                    stop_frame = min(start_frame + segment_size - 1, frame_count - 1)
                    yield SegmentParams(start_frame, stop_frame, audio_file_index)
...
def _save_task_to_db(db_task: models.Task, *, job_file_mapping: Optional[JobFileMapping] = None):
    ...
    for segment_idx, (start_frame, stop_frame, audio_file_index) in enumerate(segments):
        ...
        db_segment.audio_file_index = audio_file_index
        db_segment.save()
...
def _create_thread(
    ...
    *,
    ...
    def get_audio_duration_and_frame_count(file_path):
        ...
        container.close()
        return duration_milliseconds, total_frame

    db_task.audio_total_duration = []
    db_task.total_frames_count = []

    # calculate chunk size if it isn't specified
    if MEDIA_TYPE == "audio":
        for file_path in details['source_path']:
            duration_milliseconds, total_frame = get_audio_duration_and_frame_count(file_path)
    ...
    if db_data.storage_method == models.StorageMethodChoice.FILE_SYSTEM or not settings.USE_CACHE:
        def generate_chunks_with_overlap(
                extractor,
                files_path: List[str],
                audio_total_duration: List[int],
                total_frames_count: List[int],
                segment_duration: int,
                overlap: int,
                total_jobs_count: List[int]
            ):

            chunk_idx = 0

            for idx, file_path in enumerate(files_path):
                duration_ms = audio_total_duration[idx]
                frame_count = total_frames_count[idx]
                current_segment_duration = min(segment_duration, duration_ms)

                if duration_ms == 0 or frame_count == 0:
                    total_jobs_count.append(0)
                    continue

                frames_per_ms = frame_count / duration_ms
                segment_size = min(int(frames_per_ms * current_segment_duration), frame_count)

                step = segment_size - overlap
                num_jobs = max(1, math.ceil((frame_count - overlap) / step))
                total_jobs_count.append(num_jobs)

                chunk = []

                with extractor._get_av_container(idx) as container:
        ...
        generator = generate_chunks_with_overlap(
            extractor,
            details['source_path'],
            db_task.audio_total_duration,
            db_task.total_frames_count,
            db_task.segment_duration,
            db_task.overlap,
            db_task.total_jobs_count
        )
        ...
        def save_chunks(
            ...
            fs_original = executor.submit(
                ...
                peak_path=db_data.get_peaks_path(chunk_idx),
            )
