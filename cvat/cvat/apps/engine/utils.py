...
class AudioSegmentHandler:
    _instance = None
    _last_update_time = 0
    _update_interval = 24 * 60 * 60  # Update every 24 hours
    _update_lock = threading.Lock() # Lock for thread safety

    def __new__(cls, data_dir):
        if cls._instance is None:
            cls._instance = super(AudioSegmentHandler, cls).__new__(cls)
    ...
    def _update_model(self):
        ...
        if current_time - self._last_update_time > self._update_interval:
            with self._update_lock: # Acquire lock before updating
                if current_time - self._last_update_time > self._update_interval:
                    ...
                    try:
                        ...
                        self.model, self.utils = temp_model, temp_utils
                        self.get_speech_timestamps, self.read_audio = (
                            self.utils[0], self.utils[2]
                        )
                    ...
                    print("Model update skipped. Within update interval.")

    def get_audio_segments(self, index, min_duration=0.5):
        current_time = time.time()
        if current_time - self._last_update_time > self._update_interval:
            self._update_model()
        ...
        for next_segment in speech_timestamps[1:]:
            current_end = current_segment['end'] / 16000
            next_start = next_segment['start'] / 16000

            if (next_start - current_end) < min_duration:
                current_segment['end'] = max(current_segment['end'], next_segment['end'])
            else:
                merged_segments.append((current_segment['start'] / 16000, current_segment['end'] / 16000))
                current_segment = next_segment

        merged_segments.append((current_segment['start'] / 16000, current_segment['end'] / 16000))

        validated_segments = [
            (max(0, start), min(end, total_duration)) for start, end in merged_segments
        ]
