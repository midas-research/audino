...
class AudioReader(IMediaReader):
    ...
    def get_progress(self, pos, audio_idx=0):
        ...
        return min(1, pos / duration) if duration else None

    def _get_av_container(self, file_path_idx = 0):
        if isinstance(self._source_path[file_path_idx], io.BytesIO):
            self._source_path[file_path_idx].seek(0) # required for re-reading

        encoding = self.get_file_encoding(self._source_path[file_path_idx])
        if encoding:
            return av.open(self._source_path[file_path_idx], metadata_encoding = encoding)
        else:
...
class IChunkWriter(ABC):
    ...
    @abstractmethod
    def save_as_chunk(self, images, chunk_path, peak_path=None):
        pass
...
class AudioChunkWriter(IChunkWriter):
    ...
    def generate_and_normalize_waveform(self, chunk_path, peak_path):
        """
        Generates a waveform JSON file and then normalizes it.
        """
        try:
            subprocess.run([
                "audiowaveform",
                ...
                "--bits", "8"
            ], check=True, capture_output=True) # Using capture_output to hide console spam

            # Normalize the generated JSON file
            self.scale_json(peak_path)

        except subprocess.CalledProcessError as e:
            print(f"Failed to run audiowaveform for {chunk_path}.")
            print(f"Stderr: {e.stderr.decode()}")
            raise Exception(f"Failed to generate waveform for {chunk_path}. Please ensure audiowaveform is installed and available in PATH.") from e
        except Exception as e:
            print(f"An error occurred during waveform processing: {e}")
            raise Exception(f"An error occurred during waveform processing for {chunk_path}.") from e

    def save_as_chunk(self, images, chunk_path, peak_path):
        if not images:
        ...
        self.generate_and_normalize_waveform(chunk_path, peak_path)
        return [(0, 0)]
