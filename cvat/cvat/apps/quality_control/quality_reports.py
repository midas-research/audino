...
class AudioDatasetComparator:
    ...
    def _interval_iou(self, interval1, interval2): #(gt, ds)
        ...
        for gt_ann in gt_annotations:
            ...
            for ds_ann in ds_annotations:
                ...
                if gt_ann["label_id"] == ds_ann["label_id"]:
                    if iou >= self.settings.iou_threshold and iou > best_match_iou:
                        best_match_iou = iou
                        best_match = ds_ann
                else:
                    if iou >= self.settings.iou_threshold and iou > best_mismatch_iou:
                        best_mismatch_iou = iou
                        best_mismatch = ds_ann

            if best_match:
                matches.append((gt_ann, best_match))
                pairwise_distances[(id(gt_ann), id(best_match))] = best_match_iou
                ...
                if wer < self.settings.wer_threshold and cer < self.settings.cer_threshold:
                    mismatches.append((gt_ann, best_mismatch))
                    pairwise_distances[(id(gt_ann), id(best_mismatch))] = best_mismatch_iou
                    if gt_ann in gt_unmatched:
                        gt_unmatched.remove(gt_ann)
    ...
    def _find_audio_gt_conflicts(self):
        # Check if any segment in gt job is in ds job
        if self._job_duration <= 0:
...
class QualityReportUpdateManager:
    ...
    def _compute_reports(self, task_id: int) -> int:
        ...
        offset = 0.0  # The cumulative length (in seconds) of ground truth jobs processed before the current job.
        ...
        for i, job in enumerate(jobs):
            ...
            if task.data.original_chunk_type == DataChoice.AUDIO:
                comparator = AudioDatasetComparator(
                    ...
                    offset,
                    (time_stamps[2*i + 1] / 1000), # The duration (in seconds) of the current job that overlaps with the ground truth job.
                    settings=quality_params,
                )
                job_comparison_reports[job.id] = comparator.generate_audio_report()
                offset += (time_stamps[2*i + 1] / 1000)
            else:
        ...
        gt_job_count = np.count_nonzero(time_stamps)
